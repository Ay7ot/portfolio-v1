...
export default function MainHeader({ isLightMode }) {
  return (
    <section className='flex flex-col md:max-w-[600px] h-[80vh] md:h-[100vh]'>
        ...
        <h1 className={`mt-5 font-bold ${isLightMode ? 'text-slate-700' : 'text-white'} tracking-wider text-[2rem] md:text-[3rem] font-Taviraj`}>
            <span className={`${isLightMode ? 'text-red-600' : 'text-blue-200'}`}>Ayo</span>mide Ibiteye
        </h1>
        <h1  className={`mt-5 font-bold ${isLightMode ? 'text-slate-700' : 'text-white'} tracking-wider text-[2rem] md:text-[3rem] font-Taviraj flex flex-col`}>
